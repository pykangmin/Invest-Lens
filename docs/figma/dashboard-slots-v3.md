...
| # | 카드 위치 (절대 x) | 라벨 | 본문 | 색 (시안) | 매핑 |
|---|---|---|---|---|---|
| CT-1 | 195 | `오늘 종합 점수` | `74.2` + `전일 대비 4.20 (+0.31%)` | 녹 `#60c846` | **펀더 종합** = G1 점수 |
| CT-2 | 573 | `이번 달 종합 점수` | `74.2` + `전일 대비 4.20 (+0.31%)` | 녹 `#60c846` | **30일 종합 점수 평균** (모든 게이지 30일 추이 평균) |
| CT-3 | 947 | `오늘 종합 점수` | `65.2` + `전일 대비 4.20 (+0.31%)` | 적 `#c1121f` | **거시·기술 종합** = (G2+G3+G4) / 3 |

> 시안 라벨이 1·3 모두 `오늘 종합 점수` 인데 점수·색이 다르다 → 디자이너의 mock 라벨 중복 추정. analysis 가 두 다른 도메인 점수를 산출해 매핑. (§미해결 3)

> v2 의 "3종 평균 지수 / 펀더·기술·거시 가로 3 게이지" 와는 의미가 다르다 — 트리는 시점 비교 (오늘 / 이번 달 / 오늘 다른 도메인) 형태. 본 문서는 **"종합 점수 시점 비교 trio"** 로 재정의.
